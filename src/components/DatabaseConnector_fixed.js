...
const DatabaseConnector = ({ onConnection }) => {
  ...
  const loadSchemaInfo = async () => {
    try {
      ...
      setSchemaInfo({
        database_name: 'Connection Error',
        tables: [],
        error: error.message
      });
    } finally {
    ...
    <div className="database-connector">
      <div className="connector-header">
        ...
        <div className={`connection-badge ${connectionStatus}`}>
          ...
          {connectionStatus === 'connected' ? 'Connected' : connectionStatus === 'waiting' ? 'Waiting for Upload' : 'Disconnected'}
        </div>
      </div>
